...
for degree in range(1, max_degree + 1):
    ...
    MSE_a[degree-1], bias_a[degree-1], var_a[degree-1] = bootstrap(X_train, X_test, z_train, z_test, n, 'Ridge')
...
plt.title("Bias-variance trade-off. N = %i, noise = %.2f, %i bootstraps"%(N, noise, n), fontsize="large")
plt.grid()
plt.legend()
plt.semilogy()
plt.show()
